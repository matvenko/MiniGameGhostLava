...
#region Rendering
        void BeginCameraRendering(ScriptableRenderContext context, Camera camera)
        {
            ...
            // Profiling command
            // GetHashCode() instead of GetInstanceID(): the latter is obsolete-as-error
            // in this Unity version, and this is only a profiler label.
            CommandBuffer cmd = CommandBufferPool.Get($"Mirror {gameObject.GetHashCode()}");
            using (new ProfilingSample(cmd, $"Mirror {gameObject.GetHashCode()}"))
            {
