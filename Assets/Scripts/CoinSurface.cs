...
public class CoinSurface : MonoBehaviour
{
    ...
    // straight radial one. The two faces share u and mirror v, which is the way
    // a real coin is struck: turn the disc over about a horizontal axis and the
    // second face comes up the right way round instead of upside down. The coin
    // tumbles through every angle in play, so it is that relation between the
    // faces, rather than any one camera angle, that keeps the ghost reading
    // upright and unmirrored whichever side is turned towards the player.
    private static Vector2 CapUV(Vector3 p, bool up) =>
        new Vector2(0.5f - p.x / (2f * Radius),
                    0.5f + (up ? -p.z : p.z) / (2f * Radius));
}
